...
class FED3MonitorApp:
    def __init__(self, root):
        ...
        # Dictionary to store open serial references
        self.port_to_serial = {}

        # Track time sync commands: {port: ('pending', start_time)} or ('done', time)
    ...
    def detect_serial_ports(self):
        ...
        fed3_ports = []
        for port in ports:
            # Check for Adafruit/Arduino VID/PID used by the FED3 boards
    ...
    def setup_gui(self):
        ...
        # Start / Stop / Sync
        self.start_button = tk.Button(self.top_frame, text="START", bg="green", fg="white", font=("Cascadia Code", 12, "bold"), command=self.start_logging)
        self.start_button.grid(column=1, row=3, padx=5, pady=5)
        self.stop_button = tk.Button(self.top_frame, text="STOP and SAVE", bg="black", fg="red", font=("Cascadia Code", 12, "bold"), command=self.stop_logging)
        self.stop_button.grid(column=2, row=3, padx=5, pady=5)

        # Sync Time Button
        self.sync_time_button = tk.Button(self.top_frame, text="Sync FED3 Time", bg="blue", fg="white", font=("Cascadia Code", 12, "bold"), command=self.sync_all_device_times)
        self.sync_time_button.grid(column=0, row=3, padx=5, pady=5)

        # Dark Mode Toggle
        self.dark_mode_button = tk.Button(self.top_frame, text="Toggle Dark Mode", command=self.toggle_dark_mode, font=("Cascadia Code", 10))
        ...
        if self.serial_ports:
            ...
            tk.Label(self.ports_frame, text="Connect your FED3 units and restart RTFED!", font=("Cascadia Code", 20), fg="red").pack()
        ...
        self.footer_label = tk.Label(self.bottom_frame, text="© 2024 McCutcheonLab | UiT | Norway", font=("Cascadia Code", 10), fg="royalblue")
    ...
    def show_instruction_popup(self):
        messagebox.showinfo("Instructions", "1)Have you flashed your FED3 with RTFED library? You should be able to see RTS label on your FED3 screen.\n2) First,make a poke or trigger the pellet sensor on each FED3 to identify device number.\n3)You can press [Sync FED3 Time] button to synchronize all your FEDs time!")
        messagebox.showwarning("Caution", "1)If you need to restart a FED3 during an experiment (e.g. after fixing a jam), we recommend doing it while the internet connection is on.\n2)If needed, restart and reconnect your FED3s one at a time!\n3)IT IS VERY IMPORTANT TO IDENTIFY FED3s before pressing START or else RTFED will not log data.\n4) We recommend using a reliable powered USB hub if you are connecting many FED3 units to your computer!")
    ...
    def initialize_port_widgets(self, port, idx=None):
        ...
        frame = tk.LabelFrame(self.ports_frame, text=f"Port {port}", highlightthickness=0, bd=1, font=("Cascadia Code", 10, "bold"))
        frame.grid(column=idx % 2, row=idx // 2, padx=10, pady=10, sticky=tk.W)

        status_label = tk.Label(frame, text="Not Ready", font=("Cascadia Code", 10, "italic"), fg="red", bd=0, highlightthickness=0)
        ...
        def attempt_connection(retries=self.retry_attempts, delay=self.retry_delay):
            for attempt in range(retries):
                try:
                    ...
                    self.port_to_serial[port] = ser  # Keep port open
    ...
    def _join_threads_and_exit(self):
        ...
        # Close all open ports after logging
    ...
    def read_from_port(self, ser, worksheet_name, port_identifier):
        ...
        while not self.stop_event.is_set():
            ...
            # Check if we are awaiting time sync response for this port
            ...
            if cmd_info and cmd_info[0] == 'pending':
                ...
                if line == "TIME_SET_OK":
                    ...
                    self.log_queue.put(f"Time sync command sent to {port_identifier}, no confirmation.")
                    self.time_sync_commands[port_identifier] = ('done', time.time())
                    # We will still try to parse this line as event data if possible
                    # but if it doesn't parse, we ignore it
                # Try to parse as event data if not command response
                if line:
                    ...
                    # If mismatch here, just ignore (no warning) since we might be mixing command lines
                # No else needed: we ignore non-matching lines silently to prevent mismatch warnings
            else:
                # Normal operation (no pending command response)
                if line:
                    data_list = line.split(",")[1:] if "," in line else []
                    if len(data_list) == len(column_headers) - 1:
                        event_value = data_list[event_index].strip()
                        ...
                        # Warn only if not in pending command mode
    ...
    def sync_all_device_times(self):
        # Send current PC time to all identified devices
        now = datetime.datetime.now()
        time_str = f"SET_TIME:{now.year},{now.month},{now.day},{now.hour},{now.minute},{now.second}"

        if self.logging_active:
            # If logging is active, use already open serial connections
