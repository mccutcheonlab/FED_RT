...
Latest feature May 2025: FED3 listens to serial commands from the host computer to set the mode. The command should be in the format "SET_MODE:<0-12>" where <0-12> is the mode number. The FED3 will respond with "MODE_SET_OK" if the mode was set successfully, or "MODE_SET_FAIL" if the mode was not set successfully.
@htbibalan@uit.no
...
void FED3::writeHeader() {
  ...
  // Fixed, canonical header (Temp/Humidity always present)
  String header =
    "MM:DD:YYYY hh:mm:ss,"               // device timestamp
    "Temp,Humidity,"                     // always listed, even if blanks at runtime
    "Library_Version,Session_type,Device_Number,"
    "Battery_Voltage,Motor_Turns,FR,Event,Active_Poke,"
    "Left_Poke_Count,Right_Poke_Count,Pellet_Count,Block_Pellet_Count,"
    "Retrieval_Time,InterPelletInterval,Poke_Time,"
    "PelletsOrTrialToSwitch,Prob_left,Prob_right,High_prob_poke"; // always present
...
///////////////////////// Changing to always log temp and humidtiy field, even if blank, CHANGES AFTER PAPER REVIEW/////////////////////
    ///////////////////////////////
    /// Log temp and humidity (always output two fields)
    ///////////////////////////////
    if (tempSensor == true) {
        sensors_event_t humidity, temp;
        aht.getEvent(&humidity, &temp); // fresh data
        logData += String(temp.temperature) + "," + String(humidity.relative_humidity) + ",";
    } else {
        logData += ","; // Temp blank
        logData += ","; // Humidity blank
    }
