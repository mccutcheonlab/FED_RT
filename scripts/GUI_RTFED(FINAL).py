...
class FED3MonitorApp:
    ...
    def start_identification_thread(self, port):
        ...
        t = threading.Thread(target=self.identification_thread, args=(port, stop_event))
        t.daemon = True
        t.start()
    ...
    def stop_identification_threads(self):
        ...
        for port, event in self.identification_stop_events.items():
            event.set()
        for port, t in self.identification_threads.items():
            t.join()
            self.log_queue.put(f"Stopped identification thread for {port}.")
        self.identification_threads.clear()
        self.identification_stop_events.clear()
    ...
    def on_closing(self):
        ...
        if not self.data_saved:
            ...
            for t in self.identification_threads.values():
                t.join()
