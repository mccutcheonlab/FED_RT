...
class FED3MonitorApp:
    ...
    def setup_gui(self):
        # Instead of ttk frames, use tk frames so we can control background directly
        ...
        # Use tk.Entry to fully control color
    ...
    def show_instruction_popup(self):
        messagebox.showinfo("Instructions", "1)Have you flashed your FED3 with RTFED library? You should be able to see RTFED label on your FED3 screen.\n2) First,make a poke or trigger the pellet sensor on each FED3 to identify device number")
    ...
    def initialize_port_widgets(self, port, idx=None):
        ...
        # Use tk.LabelFrame so we can set bg and fg
    ...
    def apply_theme(self):
        ...
        for port, widgets in self.port_widgets.items():
            ...
            frame.configure(fg=fg_color)  # frame text color (for LabelFrame)
        ...
        # Update circle and label colors if needed
        if self.dark_mode:
            # If currently logging or standby, adjust label color
            # Just re-apply what's currently displayed
            circle_fill = self.canvas.itemcget(self.recording_circle, 'fill')
            if circle_fill == "Orange":  # standby
                self.canvas.itemconfig(self.recording_label, fill="white")
            elif circle_fill == "yellow":  # logging
                self.canvas.itemconfig(self.recording_label, fill="black")  # visible on dark background?
            elif circle_fill == "red":  # off
                self.canvas.itemconfig(self.recording_label, fill="red")
        else:
            # Light mode defaults
            self.canvas.itemconfig(self.recording_label, fill="black")
